...
routes.post('/askMembers', (req, res) => {
  ...
  const by = req.session.passport.user
...
routes.post('/allowMember', (req, res) => {
  const { teamName, by } = req.body
  const user = req.session.passport.user
  const output = addon.addMember(teamName, user, true)
  if (output.error === '') {
    if (output.message === 'member added successfully') {
      if (output.allMemberJoined) {
...
routes.post('/openTeamDrive', (req, res) => {
  ...
  if (teamDriveToOpen[user] !== undefined) {
    const timestamp = teamDriveToOpen[user].timestamp
    if (Date.now() - timestamp < 3600000) {
      const members = teamDriveToOpen[user].details.members
      const teamName = teamDriveToOpen[user].details.teamName
      const threshold = teamDriveToOpen[user].details.threshold

      if (!command.isTeamDriveOpen(teamName)) {
        const queryString = formQuery(members, teamName)
